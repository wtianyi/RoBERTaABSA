...
from fastNLP import (AccuracyMetric, BucketSampler, ClassifyFPreRecMetric,
                     ...
                     cache_results, Tester, ConfusionMatrixMetric)
...
# test_data_bundle = DataPipe(model_name=args.model_name, mask=mask).process_from_file(os.path.join(args.data_dir, args.test_dataset))
...
def main(args):
    ...
    # @cache_results(
    #     f"{root_fp}/caches/data_{args.test_dataset}_{mask}_{args.model_name}.pkl",
    #     _refresh=False,
    # )
    def get_data():
        data_bundle = DataPipe(model_name=args.model_name, mask=mask).process_from_file(
            os.path.join(args.data_dir, args.test_dataset)
        )
        return data_bundle
    ...
    if args.checkpoint is None:
        args.checkpoint = os.path.join(train_save_folder, os.listdir(train_save_folder)[0])
    ...
    tester = Tester(
        test_data,
        model,
        [AccuracyMetric(), ClassifyFPreRecMetric(f_type="macro"), ConfusionMatrixMetric()],
        batch_size=args.batch_size,
    )
